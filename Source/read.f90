...
REAL(EB) :: DIAMETER, GAMMA_D,AGE,INITIAL_TEMPERATURE,HEAT_OF_COMBUSTION, &
            ...
            MASS_TRANSFER_COEFFICIENT,A_COEFF_DRAG_EXP_USER,B_COEFF_DRAG_EXP_USER,&! Martin add A_COEFF,B_COEFF that will be used in the USER_DRAG_REDUCTION 
	    A_COEFF_DRAG_HOMEMADE,B_COEFF_DRAG_HOMEMADE,N_COEFF_DRAG_HOMEMADE ! Martin add A_COEFF,B_COEFF, N_COEFF that will be used in the HOMEMADE_DRAG_REDUCTION
...
CHARACTER(LABEL_LENGTH) :: SPEC_ID,DEVC_ID,CTRL_ID,QUANTITIES(1:10),QUANTITIES_SPEC_ID(1:10),SURF_ID,DRAG_LAW,DRAG_REDUCTION_LAW,PROP_ID, & ! Martin add DRAG_REDUCTION_LAW
...
NAMELIST /PART/ ADHERE_TO_SOLID,AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                CTRL_ID,DENSE_VOLUME_FRACTION,A_COEFF_DRAG_EXP_USER,B_COEFF_DRAG_EXP_USER,& ! Martin add A_COEFF_DRAG_EXP_USER,B_COEFF_DRAG_EXP_USER that will be used in the USER_DRAG_REDUCTION
                A_COEFF_DRAG_HOMEMADE,B_COEFF_DRAG_HOMEMADE,N_COEFF_DRAG_HOMEMADE,& ! Martin add A_COEFF,B_COEFF, N_COEFF that will be used in the HOMEMADE_DRAG_REDUCTION
                DEVC_ID,DIAMETER,DISTRIBUTION,DRAG_COEFFICIENT,DRAG_LAW,DRAG_REDUCTION_LAW,& ! Martin add DRAG_REDUCTION_LAW
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   LPC%A_COEFF_DRAG_EXP_USER = A_COEFF_DRAG_EXP_USER !Martin
   LPC%B_COEFF_DRAG_EXP_USER = B_COEFF_DRAG_EXP_USER !Martin
   LPC%A_COEFF_DRAG_HOMEMADE = A_COEFF_DRAG_HOMEMADE !Martin
   LPC%B_COEFF_DRAG_HOMEMADE = B_COEFF_DRAG_HOMEMADE !Martin
   LPC%N_COEFF_DRAG_HOMEMADE = N_COEFF_DRAG_HOMEMADE !Martin
...
! Martin add Drag_reduction_law
   SELECT CASE (DRAG_REDUCTION_LAW)
      CASE('RAMIREZ','null')
         LPC%DRAG_REDUCTION_LAW = RAMIREZ_DRAG_REDUCTION
      CASE('ZHU')
         LPC%DRAG_REDUCTION_LAW = ZHU_DRAG_REDUCTION
      CASE('USER_REDUCTION')
         LPC%DRAG_REDUCTION_LAW = USER_DRAG_REDUCTION
         IF (A_COEFF_DRAG_EXP_USER<0._EB .OR. B_COEFF_DRAG_EXP_USER<0._EB) THEN
            WRITE(MESSAGE,'(A,I0,A)') 'ERROR: PART line ',N,&
                                      '.  For USER REDUCTION, i.e. user defined reduction law, the coeff A and B must be positive.',N,&
                                      '.  C_D = C_D0 * [1 - (1-A) * exp(-B*L/d)] where A and B are positive'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      CASE('HOMEMADE_REDUCTION')
         LPC%DRAG_REDUCTION_LAW = HOMEMADE_DRAG_REDUCTION
         IF (A_COEFF_DRAG_HOMEMADE<0._EB .OR. B_COEFF_DRAG_HOMEMADE<0._EB .OR. N_COEFF_DRAG_HOMEMADE<0._EB) THEN
            WRITE(MESSAGE,'(A,I0,A)') 'ERROR: PART line ',N,&
                                      '.  For HOMEMADE REDUCTION, i.e. homemade drag reduction law, the coeff A, B and N must be positive.',N,&
                                      '.  C_D = C_D0 * [(1-B) * exp(-((alpha+1-A)^N))+B] where A and B are positive'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      CASE DEFAULT
         WRITE(MESSAGE,'(A)') 'ERROR: unrecognized drag  reduction law on PART line. Do not specify anything or RAMIREZ (by default) or ZHU or USER_REDUCTION'
         CALL SHUTDOWN(MESSAGE) ; RETURN
   END SELECT
...
DRAG_LAW                 = 'null'
DRAG_REDUCTION_LAW       = 'null' ! Martin add DRAG_REDUCTION_LAW
A_COEFF_DRAG_EXP_USER    = -1._EB ! Martin add A_COEFF for user defined expo reduction law
B_COEFF_DRAG_EXP_USER    = -1._EB ! Martin add B_COEFF for user defined expo reduction law
A_COEFF_DRAG_HOMEMADE    = -1._EB ! Martin add A_COEFF for homemade defined reduction law
B_COEFF_DRAG_HOMEMADE    = -1._EB ! Martin add B_COEFF for homemade defined reduction law
N_COEFF_DRAG_HOMEMADE    = -1._EB ! Martin add N_COEFF for homemade defined reduction law
