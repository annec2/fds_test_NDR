   ...
   CASE DEFAULT
	...
	! Drag reduction model for liquid droplets
        DRAG_REDUCTION_LAW_SELECT: SELECT CASE (LPC%DRAG_REDUCTION_LAW)
           ...
           CASE (NDR_DRAG_REDUCTION)
                WAKE_VEL=1.0_EB
                 IF (LPC%LIQUID_DROPLET) THEN
                    DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
                    CALL DRAG_REDUCTION_NDR(DROP_VOL_FRAC,C_DRAG,LPC%A_COEFF_DRAG_NDR,LPC%B_COEFF_DRAG_NDR,LPC%N_COEFF_DRAG_NDR)
                 ENDIF
...
!Martin Subroutine to compute the drag reduction from the Novel Drag Reduction correlation
SUBROUTINE DRAG_REDUCTION_NDR(DROP_VOL_FRAC,C_DRAG,A_COEFF_DRAG_NDR,B_COEFF_DRAG_NDR,N_COEFF_DRAG_NDR)

REAL(EB), INTENT(INOUT) :: C_DRAG
REAL(EB), INTENT(IN)  :: DROP_VOL_FRAC,A_COEFF_DRAG_NDR,B_COEFF_DRAG_NDR,N_COEFF_DRAG_NDR
REAL(EB) :: A_COEFF,B_COEFF,N_COEFF,DRAG_REDUCTION

A_COEFF = A_COEFF_DRAG_NDR
B_COEFF = B_COEFF_DRAG_NDR
N_COEFF = N_COEFF_DRAG_NDR
DRAG_REDUCTION = (1._EB - B_COEFF)*EXP(-((DROP_VOL_FRAC+1._EB-A_COEFF)**N_COEFF)) + B_COEFF
C_DRAG =  C_DRAG * DRAG_REDUCTION

END SUBROUTINE DRAG_REDUCTION_NDR
