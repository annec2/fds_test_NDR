...
INSERT_TYPE_LOOP: DO INSERT_TYPE = 1,2
   ...
   LPC_LOOP: DO N_LPC=1,N_LPC_MAX   
      ...
      IF (INSERT_TYPE==2) THEN     
         ...
         TMP_GUESS = ONE_D%TMP(1,1) !Martin
...
IF (LPC%TMP_INITIAL>0._EB) THEN
   LP_ONE_D%TMP(0:LP_SF%N_CELLS_INI+1,1) = LPC%TMP_INITIAL ! Martin
ELSEIF (LP_SF%RAMP_T_I_INDEX > 0) THEN
   DO N=0,LP_SF%N_CELLS_INI
      ...
      LP_ONE_D%TMP(N,1)=RAMPS(LP_SF%RAMP_T_I_INDEX)%INTERPOLATED_DATA(NINT(X_POS*RAMPS(LP_SF%RAMP_T_I_INDEX)%RDT)) + TMPM !Martin
   ENDDO
   LP_ONE_D%TMP(LP_SF%N_CELLS_INI+1,1) = LP_ONE_D%TMP(LP_SF%N_CELLS_INI,1) !Martin Martin
ELSEIF (LP_SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. LP_SF%TMP_INNER(1)>0._EB) THEN
   DO I=0,LP_SF%N_CELLS_INI+1
      IF (LP_SF%TMP_INNER(LP_SF%LAYER_INDEX(I))>0._EB) LP_ONE_D%TMP(I,1) = LP_SF%TMP_INNER(LP_SF%LAYER_INDEX(I)) !Martin
   ENDDO
ELSE
   LP_ONE_D%TMP(0:LP_SF%N_CELLS_INI+1,1) = TMPA !Martin
ENDIF

!Martin
IF (LPC%TMP_INNER_INITIAL>0._EB) THEN
   LP_ONE_D%TMP_INNER(0:LP_SF%N_CELLS_INI+1,2) = LPC%TMP_INNER_INITIAL
ELSEIF (LPC%TMP_INITIAL>0._EB) THEN
   LP_ONE_D%TMP_INNER(0:LP_SF%N_CELLS_INI+1,2) = LPC%TMP_INITIAL
ELSE
   LP_ONE_D%TMP(0:LP_SF%N_CELLS_INI+1,2) = TMPA
ENDIF

LP_ONE_D%TMP_F = LP_ONE_D%TMP(1,1) !Martin
   ...
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
         ...
         KILL_RADIUS_CHECK: IF (LP_ONE_D%X(1)>LPC%KILL_RADIUS) THEN
            ...
            TMP_DROP = LP_ONE_D%TMP(1,1) !Martin
	    TMP_DROP_IN = LP_ONE_D%TMP(1,2) !Martin
...
!?Martin??
               ...
               LP_ONE_D%LAYER_THICKNESS(1) = LP_ONE_D%X(1)
               LP_ONE_D%TMP(1,1) = TMP_DROP_NEW !Martin
               LP_ONE_D%TMP_F  = TMP_DROP_NEW
	       LP_ONE_D%TMP(1,2) = TMP_DROP_IN_NEW
               LP%MASS = M_DROP
