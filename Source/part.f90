...
DRAG_LAW_SELECT: SELECT CASE (LPC%DRAG_LAW)
   ...
   CASE DEFAULT
	...
	DRAG_REDUCTION_LAW_SELECT: SELECT CASE (LPC%DRAG_REDUCTION_LAW)

	   CASE (RAMIREZ_DRAG_REDUCTION)
	         WAKE_VEL=1.0_EB
	         IF (LPC%LIQUID_DROPLET) THEN
	   ...
	   CASE (ZHU_DRAG_REDUCTION)
	         WAKE_VEL=1.0_EB
	         IF (LPC%LIQUID_DROPLET) THEN
	            DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
	            IF (DROP_VOL_FRAC > LPC%DENSE_VOLUME_FRACTION) CALL DRAG_REDUCTION_ZHU(DROP_VOL_FRAC,LP%RE,C_DRAG)
	         ENDIF		

	   CASE (USER_DRAG_REDUCTION)
                WAKE_VEL=1.0_EB
		IF (LPC%LIQUID_DROPLET) THEN
	            DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
	            IF (DROP_VOL_FRAC > LPC%DENSE_VOLUME_FRACTION) CALL DRAG_REDUCTION_USER(DROP_VOL_FRAC,C_DRAG,LPC%A_COEFF_DRAG_EXP_USER,LPC%B_COEFF_DRAG_EXP_USER)
	         ENDIF

	   CASE (HOMEMADE_DRAG_REDUCTION)
                WAKE_VEL=1.0_EB
		IF (LPC%LIQUID_DROPLET) THEN
	            DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
	            IF (DROP_VOL_FRAC > LPC%DENSE_VOLUME_FRACTION) CALL DRAG_REDUCTION_HOMEMADE(DROP_VOL_FRAC,C_DRAG,LPC%A_COEFF_DRAG_HOMEMADE,LPC%B_COEFF_DRAG_HOMEMADE,LPC%N_COEFF_DRAG_HOMEMADE)
	         ENDIF

	END SELECT DRAG_REDUCTION_LAW_SELECT
...
!Martin Subroutine to compute the drag reduction based on the paper of Zhu (Zhu 1993)
SUBROUTINE DRAG_REDUCTION_ZHU(DROP_VOL_FRAC,RE,C_DRAG)

REAL(EB), INTENT(INOUT) :: C_DRAG
REAL(EB), INTENT(IN)  :: DROP_VOL_FRAC,RE
REAL(EB) :: ADIM_DIST,A_COEFF,B_COEFF,DRAG_REDUCTION

ADIM_DIST = (PI/(6._EB*DROP_VOL_FRAC))**ONTH
A_COEFF = 1._EB - EXP(-0.483_EB + 3.45E-3_EB*RE - 1.07E-5_EB*(RE**2_EB))
B_COEFF = -0.115_EB - 8.75E-4_EB*RE + 5.61E-7_EB*(RE**2_EB)
DRAG_REDUCTION = 1._EB - (1._EB - A_COEFF)*EXP(B_COEFF*ADIM_DIST)
C_DRAG =  C_DRAG * DRAG_REDUCTION

END SUBROUTINE DRAG_REDUCTION_ZHU


!Martin Subroutine to compute the drag reduction from a user defined exponential form
SUBROUTINE DRAG_REDUCTION_USER(DROP_VOL_FRAC,C_DRAG,A_COEFF_DRAG_EXP_USER,B_COEFF_DRAG_EXP_USER)

REAL(EB), INTENT(INOUT) :: C_DRAG
REAL(EB), INTENT(IN)  :: DROP_VOL_FRAC,A_COEFF_DRAG_EXP_USER,B_COEFF_DRAG_EXP_USER
REAL(EB) :: ADIM_DIST,A_COEFF,B_COEFF,DRAG_REDUCTION

ADIM_DIST = (PI/(6._EB*DROP_VOL_FRAC))**ONTH
A_COEFF = A_COEFF_DRAG_EXP_USER
B_COEFF = B_COEFF_DRAG_EXP_USER
DRAG_REDUCTION = 1._EB - (1._EB - A_COEFF)*EXP(-B_COEFF*ADIM_DIST) ! Attention, compare to ZHU, I have added a - sig in the exp in front of B to have a positive coefficient
C_DRAG =  C_DRAG * DRAG_REDUCTION

END SUBROUTINE DRAG_REDUCTION_USER


!Martin Subroutine to compute the drag reduction from the homemade correlation
SUBROUTINE DRAG_REDUCTION_HOMEMADE(DROP_VOL_FRAC,C_DRAG,A_COEFF_DRAG_HOMEMADE,B_COEFF_DRAG_HOMEMADE,N_COEFF_DRAG_HOMEMADE)

REAL(EB), INTENT(INOUT) :: C_DRAG
REAL(EB), INTENT(IN)  :: DROP_VOL_FRAC,A_COEFF_DRAG_HOMEMADE,B_COEFF_DRAG_HOMEMADE,N_COEFF_DRAG_HOMEMADE
REAL(EB) :: A_COEFF,B_COEFF,N_COEFF,DRAG_REDUCTION

ADIM_DIST = (PI/(6._EB*DROP_VOL_FRAC))**ONTH
A_COEFF = A_COEFF_DRAG_HOMEMADE
B_COEFF = B_COEFF_DRAG_HOMEMADE
N_COEFF = N_COEFF_DRAG_HOMEMADE
DRAG_REDUCTION = (1._EB - B_COEFF)*EXP(-((DROP_VOL_FRAC+1._EB-A_COEFF)^N_COEFF)) + B_COEFF
C_DRAG =  C_DRAG * DRAG_REDUCTION

END SUBROUTINE DRAG_REDUCTION_HOMEMADE
