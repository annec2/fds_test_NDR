        ...
        DRAG_REDUCTION_LAW_SELECT: SELECT CASE (LPC%DRAG_REDUCTION_LAW)
           ...
           CASE (USER_DRAG_REDUCTION)
                WAKE_VEL=1.0_EB
                 IF (LPC%LIQUID_DROPLET) THEN
                    DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
                    IF (DROP_VOL_FRAC > LPC%DENSE_VOLUME_FRACTION) THEN
                    CALL DRAG_REDUCTION_USER(DROP_VOL_FRAC,C_DRAG,LPC%A_COEFF_DRAG_EXP_USER,LPC%B_COEFF_DRAG_EXP_USER)
                 ENDIF
           ...
           CASE (HOMEMADE_DRAG_REDUCTION)
                WAKE_VEL=1.0_EB
                 IF (LPC%LIQUID_DROPLET) THEN
                    DROP_VOL_FRAC = MIN(1._EB,AVG_DROP_DEN(IIG_OLD,JJG_OLD,KKG_OLD,LPC%ARRAY_INDEX)/LPC%DENSITY)
                    CALL DRAG_REDUCTION_HOMEMADE(DROP_VOL_FRAC,C_DRAG,LPC%A_COEFF_DRAG_HOMEMADE,LPC%B_COEFF_DRAG_HOMEMADE,LPC%N_COEFF_DRAG_HOMEMADE)
                 ENDIF
