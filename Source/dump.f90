...
!Martin
   CASE(552)  ! OUTER LAYER TEMPERATURE
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%TMP_OUT - TMPM
   CASE(553)  ! INNER LAYER TEMPERATURE
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%TMP_IN - TMPM
   CASE(554)  ! OUTER LAYER DEPTH
      PARTICLE_OUTPUT = 1.E6*BOUNDARY_ONE_D(LP%OD_INDEX)%DELTA_OUT
   CASE(555)  ! INNER LAYER RADIUS
      PARTICLE_OUTPUT = 1.E6*BOUNDARY_ONE_D(LP%OD_INDEX)%R_IN
   CASE(556)  ! LATENT HEAT OF VAPORIZATION
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%H_V
   CASE(557)  ! M_DOT
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%M_DOT
!Martin stop
